...
from text.symbols import symbols, word_boundary_symbol
...
def text_to_phones(text: str, add_word_boundary=False) -> list:
    ...
    for block in blocks:
        ...
        text = block.group(2)
        if language == 'P':
            phonemes += pinyin_to_phonemes(text, add_word_boundary=add_word_boundary)
        if language == 'JA':
            phonemes += ja_to_phonemes(text)
        elif language == 'ZH':
            phonemes += zh_to_phonemes(text, add_word_boundary=add_word_boundary)
        elif language == 'EN':
            phonemes += en_to_phonemes(text, add_word_boundary=add_word_boundary)
    remaining_text = original_text[last_end:]
    phonemes += others_to_phonemes(remaining_text, add_word_boundary=add_word_boundary)
    phonemes = remove_invalid_phonemes(phonemes)
    if add_word_boundary:
        phonemes.append(word_boundary_symbol)
    return phonemes
...
if __name__ == '__main__':
    ...
    # text = "A I人工智能"
    # text = '[P]pin1 yin1 zhen1 hao3 wan2[P]扎堆儿-#, 2^2'
    # text = "[JA]それより仮屋さん,例の件ですが――[JA]"
    # text = "早上好，今天是2020/10/29，最低温度是-3°C。"
    # # text = "…………"
    # print(text_to_sequence(text))
    text = "[EN]hello, I'm fine.[EN]"
    #
    # print(time.time()-t)
    print(text_to_phones(text, add_word_boundary=True))
